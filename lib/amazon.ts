...
export type Offer={asin:string;image:string|null;price:number|null;title:string|null;availability:string|null;checkedAt:string|null;url:string};
...
export async function getOffers(ids:string[]):Promise<Offer[]>{
 ...
 if(missing.length&&process.env.AMAZON_CREDENTIAL_ID&&process.env.AMAZON_CREDENTIAL_SECRET){
  try{for(let i=0;i<missing.length;i+=10){const batch=missing.slice(i,i+10);const response=await pacedFetch({itemIds:batch,marketplace:'www.amazon.de',partnerTag:'onlinestarkei-21',resources:['images.primary.large','itemInfo.title','offersV2.listings.price','offersV2.listings.availability']});if(!response.ok)throw new Error('upstream_unavailable');const data=await response.json();for(const item of data.itemResults?.items||data.itemsResult?.items||[]){const listing=item.offersV2?.listings?.find((x:{isBuyBoxWinner?:boolean})=>x.isBuyBoxWinner)||item.offersV2?.listings?.[0];const money=listing?.price?.money;cache.set(item.asin,{offer:{asin:item.asin,image:item.images?.primary?.large?.url||null,price:money?.currency==='EUR'?money.amount:null,title:item.itemInfo?.title?.displayValue||null,availability:listing?.availability?.message||null,checkedAt:new Date().toISOString(),url:affiliateUrl(item.asin)},until:Date.now()+30*60*1000});}}}catch{ /* No stale offers are shown. */ }
 }
 return safe.map(asin=>{const hit=cache.get(asin);return hit&&hit.until>Date.now()?hit.offer:{asin,image:null,price:null,title:null,availability:null,checkedAt:null,url:affiliateUrl(asin)}});
}
